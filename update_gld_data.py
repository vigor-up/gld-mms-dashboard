...
- 整合 CFTC COT 持倉數據 → 特徵向量 + breakdown
- 綜合評分：Lambda AI 60% + 技術指標 40% + COT bias
- 信心度 > 80% 才推播至三台設備
"""
...
# ── 勝率追蹤（回測用）────────────────────────────────────────
def _load_signal_history(path: str) -> list:
    """從本地 data.json 讀取歷史信號列表（按時間倒序）。"""
    ...
    try:
        with open(path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # 支援多種可能的欄位結構
        signals = data.get('signals', []) or data.get('signal_history', [])
        return signals
    except Exception:
        return []


def calc_win_rate_20(signal_history: list, lookback: int = 20) -> float | None:
    """
    計算最近 N 次信號的勝率。
    信號 "正確" 的定義：signal 包含 BUY/STRONG_BUY 且 prob_up > 50 視為「看對」；
    或 signal 包含 SELL/STRONG_SELL 且 prob_dn > 50 視為「看對」。
    若信號強度足以帶來正收益，視為 win。
    """
    if not signal_history:
        return None
    recent = signal_history[-lookback:]
    wins = 0
    for entry in recent:
        sig  = str(entry.get('signal', ''))
        prob_up = float(entry.get('prob_up', 0))
        prob_dn = float(entry.get('prob_dn', 0))
        if ('BUY' in sig and prob_up > 50) or ('SELL' in sig and prob_dn > 50):
            wins += 1
...
class GldMmsUpdater:
    ...
    def send_push(self, title, content, is_leading=False):
        ...
        for key in self.bark_keys:
            url = (f"https://api.day.app/{key}/{title}/{content}"
                   f"?icon={icon}&group={group}&isArchive=1")
            try:
    ...
    def _calc_tech_score(self, latest):
        ...
        if vdev < -1.2:   score += 15
        elif vdev > 1.2: score -= 15
    ...
    def calculate_signals(self):
        ...
        # ── COT 預先取出（用於 breakdown）─────────────────────
        cot              = self.macro.get('cot_gold', {})
        cot_score_add    = cot.get('score_add', 0)
        cot_spec_net_pct = cot.get('spec_net_pct', 0)
        ...
        for ticker, data in self.assets.items():
            ...
            if lambda_score is not None:
                cot_bias    = cot_score_add            # +10 / -10 / 0
                combined    = round(max(0, min(100,
                               lambda_score * 0.6
                             + tech_score   * 0.4
                             + cot_bias)))
                score_note  = (f"Lambda AI {lambda_score}%×60%"
                               f" + 技術 {tech_score}%×40%"
                               f" + COT {cot_bias:+.0f}")
            else:
                combined   = tech_score
            ...
            # ── COT 特徵向量（寫入 feature_vector）───────────
            cot_spec_ls_ratio = cot.get('spec_ls_ratio', 1.0)
            ...
            if radar['bear_divergence']:#/intentionally skip
                radar_msgs.append("頂背離")
            ...
            signals[ticker] = {
                ...
                # ── COT 特徵向量（整合至 XGBoost 向量）──────
                'feature_vector': {
                    'cot_score_add':     cot_score_add,
                    'cot_spec_net_pct':  cot_spec_net_pct,
                    'cot_spec_ls_ratio': cot_spec_ls_ratio,
                },
                'radar': radar,
                'smart_money': {
                    **smart_money,
            ...
            if ticker == 'GC=F' and combined > PUSH_THRESHOLD:
                ...
                cot_r = smart_money.get('cot_report', '未知')
                if signal in ['BUY', 'STRONG_BUY']:
                    self.send_push(
                        f"📈 黃金買進！{'強烈' if signal == 'STRONG_BUY' else ''}信號",
                        f"AI綜合評分:{combined}% | 價格:${price:.2f} | ETF:{etf} | COT:{cot_r}"
                    )
    ...
    def update_html(self, html_file):
        ...
        signal_history = _load_signal_history(DATA_JSON_PATH)
        win_rate_20    = calc_win_rate_20(signal_history, lookback=20)
        ...
        # 動態 breakdown（涵蓋 COT）
        model_info  = self.lambda_result.get('model', {})
        feature_cnt = model_info.get('features', '?')
        prob_up     = self.lambda_result.get('prob_up')
        prob_dn     = self.lambda_result.get('prob_dn')

        breakdown = [
            f"XGBoost AI: {self.lambda_result.get('score', '?')}%",
            f"特徵數: {feature_cnt} + COT",
            f"COT大戶偏多: {cot_spec_net_pct}% OI",
        ]
        if win_rate_20 is not None:
            breakdown.append(f"近20次勝率: {win_rate_20}%")
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            with open(html_file, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print("[SUCCESS] v5.0 COT 整合版資料已更新（feature_vector + win_rate_20）")
        else:
            print("[WARN] HTML 中找不到 data-source 標記，未更新")
